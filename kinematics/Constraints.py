...
#接触约束(点线约束)(点到线的垂直距离固定)
class c:
    def __init__(self,i=0,S_i=np.zeros((2,1)),v_i=np.zeros((2,1)),c=0):  #构造函数
        self.i=i
        self.S_i=S_i
        self.v_i=v_i
        self.c=c
    ...
    def h(self,q):
        ...
        #print(ri)
        #print(rj)
        h = ri+np.dot(self.A(q,self.i),self.S_i)
        return h
    ...
    def constraints(self,q,t):  #构造约束
        ...
        '''
        Bi = np.dot(R,self.A(q,self.i))
        phi = 1/self.vi()*np.dot((np.dot(Bi,self.v_i)).T,self.h(q))-self.c
        return phi
    ...
    def Jacobi(self,q,t,l): #计算雅可比矩阵 l=3*n,即为自由坐标数
        ...
        J = np.zeros((1,3))
        J[0,0:2]=1/self.vi()*np.dot((self.v_i).T,Bi.T)
        J[0,2] = -1/self.vi()*np.dot(np.dot((self.v_i).T,(self.A(q, self.i).T)),ri)
    ...
    def gamma(self,q,dq,t): #计算加速度右项
        ...
        phi_i = q[3*self.i-3+2,0]
        wi = dq[3*self.i+2-3,0]
        gamma = -1/self.vi()*(np.dot(np.dot((self.v_i).T,Bi.T),ri)*wi**2+2*np.dot(np.dot((self.v_i).T,(self.A(q,self.i)).T),dri)*wi)
        return gamma